...
def analysis(df_grade):
    ...
    elective_token = False  # 1-3. 교양 토큰(보류)
    hus_token = False  # 1-3-1. hus 토큰
    ppe_token = False  # 1-3-2. ppe 토큰

    # 각 학번마다 HUS, PPE, GSC 과목 배정이 다름 -> 각 년도 학사 편람에 따라 엑셀 파일을 참조해야 함
    ...
    # 1-6. 과기경(UC0901) 필수 이수
    economy_token = False  # 1-6. 과기경 토큰
    # 미션내용
    economy_content = '미이수'
    if is_completed(df_grade_code, 'UC0901'):
        # 미션내용
        economy_content = '이수'
        economy_token = True

    # 1-7. 콜로퀴움(UC9331) 2학기 필수 이수
    ...
    if physical_token_count >= 2:
        physical_token = True

    # 미션내용
    art_content = str(art_token_count) + '/2(학기)'
    physical_content = str(physical_token_count) + '/2(학기)'
    ...
    def cpltd_major_credit(your_df_grade, your_major_list):
        ...
        for element in your_major_list:
            your_total_credit += your_df_grade[your_df_grade['교과목'] == element]['학점'].sum()
            if your_total_credit >= 42:
                your_total_credit = 42
        return your_total_credit
    ...
    if my_major == major_list[0]:  # 물리전공
        ...
        nec_major_list = [['PS2101', 'PS3204'], ['PS2102'], ['PS2103', 'PS3101'],
                          ['PS3103'], ['PS3104'], ['PS3105'], ['PS3106'],
                          ['PS3107']]
        # 고전역학 및 연습I, 전자기학 및 연습I / II, 양자물리 및 연습I / II,
        # 열력학 및 통계물리, 물리실험 I, 수리물리 I
        ...
        if nec_major_token == True and my_major_credit >= 36:
            major_token = True
        ...
        nec_major_list = [['CH2101', 'CH3204', 'CM3204', 'CM501Y', 'GS2205'],
                          ['CH2102', 'CH3104', 'CM3104'], ['CH2104'],
                          ['CH2105', 'CH3102', 'CM3102'], ['CH3106', 'CM3203', 'GS2203'],
                          ['CH3107', 'CH4218', 'CM3101', 'CM4218']]
        # 분석화학, 물리화학A(물리화학 II), 유기화학I, 물리화학B,
        # 화학합성실험, 생화학I, 무기화학
        my_now_cpltd_nec_major_list = now_cpltd_major_list(nec_major_list, my_cpltd_major_list)
        my_major_credit = cpltd_major_credit(df_grade, my_cpltd_major_list)
        if len(my_now_cpltd_nec_major_list) >= len(nec_major_list):
        ...
        nec_major_list = [['BS2101'], ['BS2102'], ['BS3101'], ['BS3105'], ['BS2103', 'BS3111'],
                          ['BS3112'], ['BS2104', 'BS3113']]
        # 유기화학I, 분자생물학, 생화학II, 세포생물학, 생화학분자생물학실험
        # 세포발생생물학실험, 생화학I
        my_now_cpltd_nec_major_list = now_cpltd_major_list(nec_major_list, my_cpltd_major_list)
        my_major_credit = cpltd_major_credit(df_grade, my_cpltd_major_list)
        if len(my_now_cpltd_nec_major_list) >= len(nec_major_list):
        ...
        nec_major_list = [['MC2100'], ['MC2101', 'MC3102'], ['MC2102', 'MC3105'], ['MC2103'],
                          ['MC3212']]
        # 열역학, 고체역학, 유체역학, 동역학, 기계공학실험 I
        my_now_cpltd_nec_major_list = now_cpltd_major_list(nec_major_list, my_cpltd_major_list)
        my_major_credit = cpltd_major_credit(df_grade, my_cpltd_major_list)
        if len(my_now_cpltd_nec_major_list) >= len(nec_major_list):
            nec_major_token = True
        ...
        nec_major_list = [['MA2101', 'MA3101'], ['MA2102', 'MA3205'], ['MA2103'],
                          ['MA2104', 'MA3102'], ['MA3104'], ['MA3105']]
        # 재료과학, 열역학, 유기재료화학, 고분자과학, 전자재료실험, 유기재료실험
        my_now_cpltd_nec_major_list = now_cpltd_major_list(nec_major_list, my_cpltd_major_list)
        my_major_credit = cpltd_major_credit(df_grade, my_cpltd_major_list)
        if len(my_now_cpltd_nec_major_list) >= len(nec_major_list):
            nec_major_token = True
    ...
    elif my_major == major_list[6]:  # 환경전공

        my_cpltd_major_list = [x for x in df_grade_code if x[:2] == 'EV']
        nec_major_list = [['EV3101'], ['EV3106'], ['EV3111'], ['EV3103', 'EV4106'], ['EV4107', 'EV4206']]
        # 환경공학, 환경분석실험I, 지구시스템과학, 지구환경이동현상, 환경분석실헝 II
        my_now_cpltd_nec_major_list = now_cpltd_major_list(nec_major_list, my_cpltd_major_list)
        my_major_credit = cpltd_major_credit(df_grade, my_cpltd_major_list)
        if len(my_now_cpltd_nec_major_list) >= len(nec_major_list):
            nec_major_token = True
    ...
    elif my_major == major_list[5]:
        nec_major_content = str(len(my_now_cpltd_nec_major_list)) + '/' + str(len(nec_major_list)) + '(과목)'
        major_content = str(my_major_credit) + '/30(학점)<br>(최대 42학점 인정)'
    else:
        nec_major_content = str(len(my_now_cpltd_nec_major_list)) + '/' + str(len(nec_major_list)) + '(과목)'
        major_content = str(my_major_credit) + '/36(학점)<br>(최대 42학점 인정)'
    ...
    # 총 학점(일반 학점+전공 학점(최대 42학점)+교양 학점(최대 36학점))
    ...
    if my_major == '기초교육학부':
        graduate_mission_list = [english_token, writing_token, elective_token, basic_sci_token,
                                 freshman_token, economy_token, colloquium_token, research_token, math_token,
        ...
        mission_list = ['영어 1 및 2', '글쓰기 과목', '교양 과목', '기초과학 과목', 'GIST 새내기/신입생 세미나',
                        '과학기술과 경제', '콜로퀴움(2학기 이상)', '학논연 과목', '수학 과목', '예능 과목(2학기 이상)',
                        '체능 과목(2학기 이상)', '전공 선언']
        mission_content_list = [english_content, writing_content, elective_content, basic_sci_content, freshman_content,
                                economy_content, colloquium_content, research_content, math_content, art_content,
