...
def analysis(df_grade):
    ...
    # 미션내용
    writing_content = '글쓰기와 말하기, 고급작문 과목 미이수(대체과목 포함)'
    writing1_token = False  # 글쓰기와 말하기 토큰
    writing2_token = False  # 고급작문 토큰

    writing1_code_list = ['GS1511', 'GS1512', 'GS1513']  # 글쓰기와 말하기 대체 과목
    writing2_code_list = ['GS1531', 'GS1532', 'GS1533', 'GS1534']  # 고급작문 대체 과목

    if is_completed(df_grade_code, 'GS1501'):
        writing1_token = True

    else:
        for writing_code in writing1_code_list:
            if is_completed(df_grade_code, writing_code):
                writing1_token = True
                break

    if is_completed(df_grade_code, 'GS1502'):
        # 미션내용
        writing_content = '글쓰기와 말하기 과목(대체과목 포함) 미이수,<br>고급작문 과목(대체과목 포함) 이수'
        writing2_token = True
    else:
        for writing_code in writing2_code_list:
            if is_completed(df_grade_code, writing_code):
                writing2_token = True
                break

    if writing1_token and writing2_token:
        # 미션내용
        writing_content = '글쓰기와 말하기, 고급작문 과목 이수(대체과목 포함)'
        writing_token = True

    # 1-3. 교양(+기초교육학부 개별연구 이수학점)
    # HUS, PPE, GSC 합 24학점 이상 이수 필수
    ...
    # (16학번까지, 15학번 이상은 교양 단일 파일(모든 과목 GSC로 들어가게 처리))
    try:
    ...
    # 미션내용
    elective_content = '총 교양학점: ' + str(elective_credit) + '/24(학점)<br>(최대 36학점 인정)'
    ...
    if (is_completed(df_grade_code, 'GS1101') or is_completed(df_grade_code, 'GS1103')) and is_completed(df_grade_code,
                                                                                                         'GS1111') and (
            is_completed(df_grade_code, 'GS1102') or is_completed(df_grade_code, 'GS1104')) and is_completed(
            df_grade_code, 'GS1112'):
        # 미션내용
        basic_sci_content += '일반물리학'
        basic_physics_token = True

    # 1-4-2. 화학: 일화I / II 및 일화실 I / II 이수
    basic_chemistry_token = False  # 1-4-2. 기초화학 토큰

    if (is_completed(df_grade_code, 'GS1201') or is_completed(df_grade_code, 'GS1203')) and is_completed(df_grade_code,
                                                                                                         'GS1211') and (
            is_completed(df_grade_code, 'GS1202') or is_completed(df_grade_code, 'GS1204')) and is_completed(
            df_grade_code, 'GS1212'):
        # 미션내용
    ...
    # 1-4-3. 생명: 일생 I 또는 고급일생 I 또는 인생(일생 II) 및 일생실(GS1311) 이수

    basic_biology_token = False  # 1-4-3. 기초생물학 토큰

    if (is_completed(df_grade_code, 'GS1301') or is_completed(df_grade_code, 'GS1302') or is_completed(df_grade_code,
                                                                                                       'GS1303') or is_completed(
            df_grade_code, 'GS2301')) and is_completed(df_grade_code, 'GS1311'):
        # 미션내용
        if basic_sci_content != '':
            basic_sci_content += ', '
    ...
    # 위 조건을 모두 이수해야 함
    if basic_physics_token == True and basic_chemistry_token == True and basic_biology_token == True and basic_eecs_token == True:
    ...
    # 1-6. 콜로퀴움(UC9331)+전공세미나(GS9331) 2학기 필수 이수
    colloquium_token = False  # 1-7. 콜로퀴움 토큰
    if df_grade_code.count('UC9331') + df_grade_code.count('GS9331') >= 2:
        colloquium_token = True
    colloquium_content = str(df_grade_code.count('UC9331') + df_grade_code.count('GS9331')) + '/2(학기)'

    # 1-7. 수학
    math_token = False
    calculus_token = False
    analysis_token = False
    equation_or_algebra_token = False
    # 미션내용
    math_content = ''
    if is_completed(df_grade_code, 'GS1001') or is_completed(df_grade_code, 'GS1011'):
        ...
        math_content += '미적분학'
        calculus_token = True
    if is_completed(df_grade_code, 'GS1002') or is_completed(df_grade_code, 'GS1012'):
        # 미션내용
        math_content += ', 다변수해석학'
        analysis_token = True
    if is_completed(df_grade_code, 'GS2002') or is_completed(df_grade_code, 'GS2004'):
        # 미션내용
        if is_completed(df_grade_code, 'GS2002'):
            math_content += ', 미분방정식'
        if is_completed(df_grade_code, 'GS2004'):
            math_content += ', 선형대수학'
        equation_or_algebra_token = True

    if calculus_token and analysis_token and equation_or_algebra_token:
        math_token = True
    ...
    if physical_token_count >= 6:
        physical_token = True

    # 미션내용
    art_content = str(art_token_count) + '/4(학기)'
    physical_content = str(physical_token_count) + '/6(학기)'
    ...
    if my_major == major_list[0]:  # 물리전공
        ...
        nec_major_list = [['MC3101', 'MC2102', 'MC3105', 'MC2100'],
                          ['MC2101', 'MC3102'],
                          ['MC3103'], ['MC3104', 'MC3208']]
        # 열유체역학(또는 열역학 또는 유체역학), 고체역학, 기구동역학, 공학설계
        my_now_cpltd_nec_major_list = now_cpltd_major_list(nec_major_list, my_cpltd_major_list)
        my_major_credit = cpltd_major_credit(df_grade, my_cpltd_major_list)
        if len(my_now_cpltd_nec_major_list) >= len(nec_major_list):
            nec_major_token = True
